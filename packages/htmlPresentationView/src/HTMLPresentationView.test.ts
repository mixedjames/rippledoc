...
describe("HTMLPresentationView integration", () => {
	...
	it("realises and lays out DOM structure for a simple presentation", async () => {
		...
		// Root structure
		const container = root.querySelector(".presentation-root") as HTMLElement | null;
		expect(container).not.toBeNull();
		expect(container!.style.position).toBe("relative");

		const backgrounds = container!.querySelector(".backgrounds") as HTMLElement | null;
		const elements = container!.querySelector(".elements") as HTMLElement | null;
